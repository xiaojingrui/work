    ...
    public static function renderScriptPool(){
        ...
        if(!empty(self::$arrScriptPool)) {
            ...
            foreach ($priorities as $priority) {
                $html .= '<script type="text/javascript">var __ready = [];__ready.push(function(){' . implode("});\n__ready.push(function(){", self::$arrScriptPool[$priority]) . '});window.initEvent && window.initEvent();</script>';
            }
        }
        return $html;
    }
