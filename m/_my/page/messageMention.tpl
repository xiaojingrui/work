...
{%block name="content"%}
    ...
    <section class="tab-wrap">
        <div class="tab">
            <a href="javascript:;" class="active">提到我的{%if $remind.at != 0%}<span class="remind">{%if $remind.at > 99%}99+{%else%}{%$remind.at%}{%/if%}</span>{%/if%}</a>
                ...
                {%foreach $messages.list as $val%}
                    ...
                    {%else if !empty($val.pid) && $val.post.quote == "0"%}
                        <a href="{%$val.thread_url%}" class="item comment-mention">
                            <div class="hd">
                                <span class="title">[帖子]&nbsp;<span class="t-name">{%$val.title%}</span>提到了你</span>
                                <span class="time">{%$val.format_time%}</span>
                            </div>
        ...
        <@ _.each(datas, function(item, index){ @>
            ...
            <@ }else if(item.pid != "0" && item.post.quote == "0"){ @>
                <a href="<@=item.thread_url@>" class="item comment-mention">
                    <div class="hd">
                        <span class="title">[帖子]&nbsp;<span class="t-name"><@=item.title@></span>提到了你</span>
                        <span class="time"><@=item.format_time@></span>
                    </div>
