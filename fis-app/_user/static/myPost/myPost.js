(function(){
    ...
    var MorePost = {
        ...
        getMorePost: function(callback) {
            var url = "/" + GM.projectId +"/"+ GM.version + "/user/getUserThreads?" + "puid="+ GM.puid + "&client=" + GM.client,
                self = this;
            var success = function(data) {
                if(data.status == 200) {
                    $('#J_content').append(_.template( $('#J-post-tpl').html(),{datas: data.result.list} ));
                    GM.page ++;
                    var hasNextPage = data.result.nextPage;
                    if(!hasNextPage) {
                        $('.my-post-wrap').append('<div class="has-more-reply"><p>由于权限限制，部分帖子可能被隐藏</p><p>没有更多</p></div>');
                        callback('finish');
                    }else{
                        if(self.flag) {
